...
3. **Images** — `ai/images.py` runs all image generations concurrently (semaphore of 4). Provider order: bfl → fal → prodia → pollinations → huggingface → PIL placeholder. Each prompt gets cinematic style modifiers appended before generation.
...
| Prodia (Flux Schnell) | Ultra-fast Image generation | `PRODIA_TOKEN` (optional) |
